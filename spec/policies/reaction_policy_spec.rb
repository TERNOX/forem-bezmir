...
RSpec.describe ReactionPolicy do
  ...
  context "when user is signed in" do
    ...
    context "when user is a support admin and the reaction is a flag on a comment" do
      let(:user) { create(:user, :support_admin) }
      let(:reaction) { create(:vomit_reaction, reactable: comment) }

      it { is_expected.to permit_actions(%i[admin_invalidate]) }
      it { is_expected.to forbid_actions(%i[admin_update privileged_create]) }
    end

    context "when user is a support admin and the comment reaction is not a flag" do
      let(:user) { create(:user, :support_admin) }
      let(:reaction) { create(:reaction, reactable: comment, category: "like") }

      it { is_expected.to forbid_actions(%i[admin_invalidate]) }
    end
