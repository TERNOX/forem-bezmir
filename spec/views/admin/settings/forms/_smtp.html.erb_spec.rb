...
RSpec.describe "admin/settings/forms/_smtp" do
  it "keeps saved fields visible and editable when automatic digests are disabled" do
    ...
    expect(form.at_css("#settings_smtp_automatic_digests_enabled[checked]")).to be_nil
    checkbox = form.at_css("#settings_smtp_automatic_digests_enabled")
    expect(checkbox["aria-describedby"]).to eq("smtp-delivery-description")
