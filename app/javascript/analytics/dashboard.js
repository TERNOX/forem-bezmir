...
function showErrorsOnCharts() {
  ...
  target.forEach((id) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.outerHTML = `<div class="m-5" id="${id}"><p>Не вдалося завантажити дані графіка. Якщо помилка не зникає за хвилину, спробуйте вимкнути блокувальник реклами на цій сторінці чи сайті.</p>${retryButtonHTML()}</div>`;
  });
  bindRetryButtons();
}
...
function showErrorsOnReferrers() {
  ...
  if (container) {
    ...
    container.innerHTML = `<tr><td colspan="2" class="p-5"><p>Не вдалося завантажити дані про джерела трафіку. Якщо помилка не зникає за хвилину, спробуйте вимкнути блокувальник реклами на цій сторінці чи сайті.</p>${retryButtonHTML()}</td></tr>`;
  }
  bindRetryButtons();
}
