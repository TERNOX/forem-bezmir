...
function showErrorsOnCharts() {
  ...
  target.forEach((id) => {
    const el = document.getElementById(id);
    if (!el) return;
    el.outerHTML = `<p class="m-5" id="${id}">Не вдалося завантажити дані графіка. Якщо помилка не зникає за хвилину, спробуйте вимкнути блокувальник реклами на цій сторінці чи сайті.</p>`;
  });
}
